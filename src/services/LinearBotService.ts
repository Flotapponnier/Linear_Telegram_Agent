...
export default class LinearTrackerBot {
  ...
  private get brandName(): string {
    return this.config.get<string>('BOT_BRAND_NAME') || 'Linear Tracker';
  }

  private get workspaceSlug(): string {
    return this.config.get<string>('LINEAR_WORKSPACE_SLUG') || 'workspace';
  }
  ...
  private getLinearIssueUrl(identifier: string): string {
    return `https://linear.app/${this.workspaceSlug}/issue/${identifier}`;
  }

  private getExampleTicketId(): string {
    return `${this.ticketPrefix}-1234`;
  }
  ...
  async launchBot() {
    ...
    // Start command
    this.bot.start((ctx) => {
      const welcomeMsg = `🚀 <b>Welcome to ${this.brandName}!</b>
...
\n\n<b>Example:</b>\n/ticket Fix login issue | Users unable to authenticate`,
        ...
        { 
          ...
          reply_markup: {
            inline_keyboard: [
              ...
              [
                { text: '🔗 Open in Linear', url: this.getLinearIssueUrl(issueIdentifier) },
              ],
            ],
          },
        },
    ...
    this.bot.action(/^done_(.+)$/, async (ctx) => {
      ...
      const issueIdentifier = ctx.match[1];
      const linearUrl = this.getLinearIssueUrl(issueIdentifier);
  ...
  private async handleEditAction(
    ...
    if (!ticketIdentifier) {
      await ctx.telegram.editMessageText(
        ...
        messageId,
        undefined,
        `❌ <b>Could not identify the ticket</b>\n\nPlease specify the ticket (e.g., "edit ${this.getExampleTicketId()}")`,
        { parse_mode: 'HTML' },
      );
      return;
    }
        ...
        { 
          ...
          reply_markup: {
            inline_keyboard: [
              ...
              [
                { text: '🔗 Open in Linear', url: this.getLinearIssueUrl(ticketIdentifier) },
              ],
            ],
          },
        },
  ...
  private async handleDeleteAction(
    ...
    if (!ticketIdentifier) {
      await ctx.telegram.editMessageText(
        ...
        messageId,
        undefined,
        `❌ <b>Could not identify the ticket</b>\n\nPlease specify the ticket (e.g., "delete ${this.getExampleTicketId()}")`,
        { parse_mode: 'HTML' },
      );
      return;
    }
  ...
  private async handleCancelAction(
    ...
    if (!ticketIdentifier) {
      await ctx.telegram.editMessageText(
        ...
        messageId,
        undefined,
        `❌ <b>Could not identify the ticket</b>\n\nPlease specify the ticket (e.g., "cancel ${this.getExampleTicketId()}")`,
        { parse_mode: 'HTML' },
      );
      return;
    }
  ...
  private async handleAssignAction(
    ...
    if (!ticketIdentifier) {
      await ctx.telegram.editMessageText(
        ...
        messageId,
        undefined,
        `❌ <b>Could not identify the ticket</b>\n\nPlease specify the ticket (e.g., "assign ${this.getExampleTicketId()} to [name]")`,
        { parse_mode: 'HTML' },
      );
      return;
    }
    ...
    if (!assigneeName) {
      await ctx.telegram.editMessageText(
        ...
        messageId,
        undefined,
        `❌ <b>Could not identify the assignee</b>\n\nPlease specify who to assign (e.g., "assign ${this.getExampleTicketId()} to [name]")`,
        { parse_mode: 'HTML' },
      );
      return;
    }
  ...
  private async handleStatusAction(
    ...
    if (!ticketIdentifier) {
      await ctx.telegram.editMessageText(
        ...
        messageId,
        undefined,
        `❌ <b>Could not identify the ticket</b>\n\nPlease specify the ticket (e.g., "set ${this.getExampleTicketId()} to In Progress")`,
        { parse_mode: 'HTML' },
      );
      return;
    }
  ...
  private async handleCreateAction(
    ...
    const linearUrl = this.getLinearIssueUrl(issue.identifier);
